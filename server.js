...
const REDIRECT_URI = process.env.REDIRECT_URI || `http://ncrpfbi.com/callback`;
...
app.get("/staff-panel", isAuthenticated, async (req, res) => {
  if (!req.user || req.user.id !== "1065934093470158918" || req.user.id !== "708001346816835624" || req.user.id !== "928586046072029216") {
...
app.get("/staff-panel", isAuthenticated, async (req, res) => {
  if (!req.user || req.user.id !== "1065934093470158918") {
    return res.redirect("/access-denied");
  }
